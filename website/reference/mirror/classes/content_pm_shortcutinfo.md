...
::: tip 源码路径
[`src/main/java/mirror/android/content/pm/ShortcutInfo.java`](https://github.com/android-security-engineer/VirtualXposed-skills/blob/vxp/VirtualApp/lib/src/main/java/mirror/android/content/pm/ShortcutInfo.java)
:::

镜像真实类 `android.content.pm.ShortcutInfo`，用 Ref 引用对象包装其隐藏成员。
...
镜像的真实类为 `android.content.pm.ShortcutInfo`（@hide 隐藏 API），被以下模块引用：
...
```mermaid
flowchart LR
  SC["影子类<br/>mirror.ShortcutInfo"] -->|"RefClass.load"| BIND["绑定真实<br/>ShortcutInfo"]
