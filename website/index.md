...
features:
  - icon: 🛡️
    title: 免 Root 运行 Xposed
    details: 不需要解锁 Bootloader、不需要刷机，在普通 App 进程内即可加载并激活 Xposed 模块，Hook 目标应用的方法调用。
  - icon: 📦
    title: 应用沙箱虚拟化
  ...
  - icon: 🗂️
    title: IO 重定向
    details: native 层 hook libc 文件接口，把 /data/data/&lt;pkg&gt;、/sdcard 等路径透明重定向到虚拟数据目录，实现数据隔离。
  - icon: ⚡
    title: ART 方法 Hook
